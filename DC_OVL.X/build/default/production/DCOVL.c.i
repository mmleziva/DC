...
uint8_t in[8], set, res, film ;
uint8_t step,stepold,i, k, j ,lt,ltnaraz, prodleva,probrzd,startime,bufout[5];
uint8_t plyn,blik,brac,pb;
uint16_t kniply,kniplx,zpozdeni;
...
void adc_filter(uint16_t *act,uint16_t *filt)
{
    int16_t ax= (int16_t)((*act)>>4) - (int16_t)((*filt)>>4);
    *filt += (uint16_t)ax;
}

uint8_t pwmakc(uint16_t uak)
{
    word uacc, resw;
    ...
    if(huac <= (256/5))
        result=0;
    else if(huac >= ((256*4)/5))
        result= 251;
    else
    {
        difhu = huac-(256/5);
        resw.w= (((251 - (((251)*15)/100-3))*5)/(4-1)) * difhu;
        result=resw.H +(((251)*15)/100-3);
    }
    return result;
}

uint8_t pwmbrzd(uint16_t uak)
{
    word uacc, resw;
    uint8_t huac, result, difhu;
    uacc.w= uak;
    ...
    {
        difhu = huac-(256/5);
        resw.w= (((251 - (((251)*20)/100))*5)/(4-1)) * difhu;
        result=resw.H +(((251)*20)/100);
    }
    return result;
}
...
int main(int argc, char** argv)
{
   ...
   kniplxfil.w= adc_read(0);
   kniplyfil.w= adc_read(1);
   ...
   while(1)
   {
     ...
     if(TMR1IF)
     {
       ...
       QBLIK= ((blik & 0x20) != 0);
       blik++;
       AKCEL_SEP=(kniplxfil.w > (0x10000/5));
       if(kom > 0)
       {
           kom--;
           RC3= LBLIK;
